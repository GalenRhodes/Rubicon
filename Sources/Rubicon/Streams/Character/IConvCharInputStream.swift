...
import CoreFoundation
    ...
    open class IConvCharInputStream: CharInputStream {
        ...
        private var inputStreamIsOpen: Bool {
            !Rubicon.value(inputStream.streamStatus, isOneOf: .closed, .notOpen)
        }
        ...
        private func doBackgroundRead() -> Bool {
            do {
                ...
                let cc = try inputStream.append(to: &buffer, maxLength: INPUT_BUFFER_SIZE)
                return (cc > 0)
            }
            catch let e {
                error = e
